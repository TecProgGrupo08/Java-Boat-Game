...
public class Swaying extends AngledAcceleration {
	public Swaying(AngledAcceleration move, double x, double y, double pRandomPhase, Character owner, double swayH,
			double swayV) {
		this.move = move;
		this.owner = owner;
		xAnchor = x;
	...
	public Location go(Location location) {
		...
		/*
		 * this routines make the boat sway based if he is moving or not if he's
		 * standing still, the sway tends to be a little larger
		 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see game.movement.AngledAcceleration#goDown(game.movement.Location) set
	 * of functions that need to be overwritten for the swaying
	 */
	@Override
