...
public class Renderer extends JPanel {
	...
	private Image background;	//Define background image 
	Collection<Character> charactersToRender; //Get all character to the map
	...
	private void initComponents() {
		...
		layout.setHorizontalGroup(
				layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 718, Short.MAX_VALUE));
		layout.setVerticalGroup(
				layout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING).addGap(0, 491, Short.MAX_VALUE));
	...
	/**
	 * update - this acts as a double buffer update is normally called before
	 * paint. the image is cleared and this results in flicker. Here we override
	 * the update method to create an image to swap behind to reduce flicker.
	 * 
	 * @param g
	 *            a Graphics object representing the current graphics context
	 */
	...
	/*
	 * private Graphics2D drawArrow(Graphics2D gg) { CharacterBoat boat =
	 * (CharacterBoat)GameEngine.getInstance().getCharacters().get(0);
	 * 
	 * Location boatLocation = boat.getLocation();
	 * 
	 * Controller c = Controller.getInstance(); arrowDrawnOnce=true; Location l
	 * =c.getMouseLocation(); int boatX= (int)boatLocation.x; int boatY=
	 * (int)boatLocation.y; int mouseX= (int)l.x; int mouseY= (int)l.y; int dx =
	 * mouseX -boatX; int dy = mouseY- boatY; if (dx==0) dx =1;//just
	 * approximate so avoids divide by zero errors and creating 0 height/width
	 * images if (dy==0) dy =1; double angle = Math.tan((int)(dy / dx));
	 * BufferedImage b = (BufferedImage)createImage(Math.abs(dx),Math.abs(dy));
	 * 
	 * Graphics2D gTemp = (Graphics2D)b.createGraphics();
	 * 
	 * Color arrow= new java.awt.Color(230,230,10,40); gTemp.setColor(arrow);
